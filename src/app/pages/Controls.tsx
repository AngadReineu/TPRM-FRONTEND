...
const CONTROLS: Control[] = [
  {
    id: '1', name: 'MFA Enforcement', desc: 'Multi-factor auth on all admin accounts',
  ...
  {
    id: '2', name: 'Encryption at Rest', desc: 'AES-256 on all storage volumes',
    category: 'Technical', active: true, coverage: 67, scope: 'Partial', risk: 'High', lastEval: '15 min ago', deps: 2,
    internalSPOC: 'anita@abc.co', externalSPOC: 'ops@abc.com',
    piiFlow: 'both', truthValidator: true, hasTruthGap: true, personality: 'Security',
  },
  {
    id: '3', name: 'Access Review Policy', desc: 'Quarterly access review',
  ...
  {
    id: '4', name: 'Network Segmentation', desc: 'VLAN isolation prod vs staging',
  ...
  {
    id: '5', name: 'Patch Management', desc: 'OS patching within 30-day SLA',
  ...
  {
    id: '6', name: 'Data Classification Policy', desc: 'All data assets classified',
  ...
  {
    id: '7', name: 'Incident Response Plan', desc: 'Documented IR with runbooks',
  ...
  {
    id: '8', name: 'Backup Verification', desc: 'Weekly backup integrity test',
  ...
  {
    id: '9', name: 'Vulnerability Scanning', desc: 'Automated weekly assessments',
  ...
  {
    id: '10', name: 'Privileged Access Mgmt', desc: 'JIT access for admin ops',
    category: 'Technical', active: true, coverage: 82, scope: 'Full', risk: 'Critical', lastEval: '5 min ago', deps: 3,
    internalSPOC: 'anita@abc.co', externalSPOC: 'john@xyz.com',
    piiFlow: 'both', truthValidator: true, hasTruthGap: false, personality: 'Security',
  },
];
...
/* ── Relational Context (SPOC hover card) ────────────── */
    ...
    >
      ...
      {hovered && (
        <div style={{
          ...
          fontSize: 12, lineHeight: 1.6, whiteSpace: 'nowrap',
          boxShadow: '0 4px 16px rgba(0,0,0,0.2)',
          pointerEvents: 'none',
        }}>
          {internal && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ color: '#94A3B8', fontSize: 10, fontWeight: 600, letterSpacing: '0.04em' }}>INT</span>
              <span style={{ color: '#93C5FD' }}>{internal}</span>
            </div>
          )}
          {external && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ color: '#94A3B8', fontSize: 10, fontWeight: 600, letterSpacing: '0.04em' }}>EXT</span>
              <span style={{ color: '#86EFAC' }}>{external}</span>
            </div>
          )}
          {/* Arrow */}
          <div style={{
...
export function Controls() {
  ...
  const [primaryTab, setPrimaryTab] = useState<'Technical' | 'Process'>('Technical');
  ...
  const filtered = CONTROLS.filter(c => {
    ...
    // Primary tab filter
    const matchTab = primaryTab === 'Technical'
      ? ['Technical', 'Document', 'Expected Res.'].includes(c.category)
      : c.category === 'Process';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: '#64748B', margin: '4px 0 0' }}>
            {CONTROLS.length} controls · {validatorCount} truth validators · {truthGapCount > 0 ? (
      ...
      <div style={{ display: 'flex', gap: 0, backgroundColor: '#fff', border: '1px solid #E2E8F0', borderRadius: 10, padding: 4, alignSelf: 'flex-start' }}>
        {([
          { id: 'Technical', label: 'Technical Controls', hint: 'MFA, Encryption, Scanning, Evidence' },
          { id: 'Process',   label: 'Process Controls',   hint: 'SOW, SLA, Payment, Audit Trail' },
        ] as const).map(tab => {
          ...
          return (
            <button
              ...
              title={tab.hint}
              style={{
                padding: '8px 20px', borderRadius: 7, fontSize: 13, fontWeight: sel ? 700 : 500, cursor: 'pointer', border: 'none',
                backgroundColor: sel ? (tab.id === 'Technical' ? '#0EA5E9' : '#10B981') : 'transparent',
                color: sel ? '#fff' : '#64748B',
                transition: 'all 0.15s',
              }}
            >
              {tab.id === 'Technical' ? '⬡ ' : '⟳ '}{tab.label}
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {['All', 'Technical', 'Process', 'Document', 'Expected Res.'].map(cat => (
            <button
              key={cat}
        ...
        <button style={{
          display: 'flex', alignItems: 'center', gap: 6,
          backgroundColor: '#fff', border: '1px solid #E2E8F0', borderRadius: 8,
          padding: '9px 14px', fontSize: 13, fontWeight: 500, color: '#334155', cursor: 'pointer',
        }}>
            ...
            <tbody>
              {filtered.map((c, idx) => (
                ...
                >
                  ...
                  {/* Personality */}
                  <td style={tdStyle}>
                    <PersonalityBadge p={c.personality} />
                  </td>
