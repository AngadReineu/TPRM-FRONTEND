...
import { getRoles, getModules, getPermissions, getUsers, getRoleColors } from '../services/roles.data';
              ...
              {users.map((u, idx) => {
                ...
                return (
                  <tr key={u.id} className={`hover:bg-slate-50 ${idx < users.length - 1 ? 'border-b border-[#F1F5F9]' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <button className="p-1.5 rounded-md bg-transparent border-none text-slate-400 cursor-pointer hover:text-sky-500 hover:bg-sky-50">
                        <Edit2 size={14} />
                      </button>
                    </td>
                  </tr>
