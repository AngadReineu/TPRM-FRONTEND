...
import {
  ...
  FileText, Flag, GitMerge,
} from 'lucide-react';
import { toast } from 'sonner';

import type { Agent, LogEntry, TaskStatus } from '../types';
...
function CreateAgentModal({ onClose, onCreated }: { onClose: () => void; onCreated: (a: Agent) => void }) {
  ...
  const handleViewAgents = () => {
    ...
    onCreated({
      id: `a${Date.now()}`, name: name.trim(), initials, status: 'active', stage: firstStage,
      controls: controls.size, suppliers: suppliers.size, gradient, alerts: 0,
      lastActive: 'just now', health: 100, division: division || 'Unassigned', frequency,
      notify: Array.from(notify), agentName: name.trim(), role: 'Custom Agent',
      color: '#0EA5E9', avatarSeed: name.trim(), uptime: '100%', nextEval: '\u2014',
      lastScan: '\u2014', openTasks: 0, currentTask: 'Idle',
    });
    onClose();
  };
          ...
          {success ? (
            <div className="flex flex-col items-center justify-center py-10 text-center gap-3.5">
              ...
              <div className="flex gap-2.5 mt-1">
                ...
                <button onClick={() => setSuccess(false)} className="bg-white text-slate-700 border border-slate-200 rounded-lg px-4 py-2.5 text-sm cursor-pointer">Create Another</button>
              </div>
            </div>
          ) : (
            <>
...
function AgentDetailView({
  ...
  const tasks      = getAgentTasksList(agent.id);
  const timeline   = getAgentTimelineList(agent.id);
  ...
  const { logs, pulse } = useAgentLogStream({
    initialLogs: getInitialLogsList(agent.id),
